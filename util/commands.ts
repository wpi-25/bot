...
export const commandAllowed = (message:Message, rule:any) => {
    if (rule && message.guild.id in rule) { // If there's a rule defined and the guild is in the rule
        switch (typeof rule[message.guild.id]) {    // And the rule for this guild is
            case 'string':  // A string,
                return message.channel.id == rule[message.guild.id];    // Check if this is the right channel
            
            case 'object':  // If it's an array,
                return (<Array<Snowflake>>rule[message.guild.id]).includes(message.channel.id); // Check if this channel is in the array
        }
    }
